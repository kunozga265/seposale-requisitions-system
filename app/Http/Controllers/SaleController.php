...
class SaleController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $sale = Cache::lock($user->id . ':sales:store', 10)->get(function () use ($user, $request, $client) {
            ...
            $sale = Sale::create([
                ...
                'balance' => $request->total + $request->vat,
        ...
        if (is_object($quotation)) {
            ...
            $sale = Cache::lock($user->id . ':sales:store', 10)->get(function () use ($quotation, $user, $request) {
                ...
                $sale = Sale::create([
                    ...
                    'local_purchase_order' => $request->local_purchase_order,
                    'vat' => $quotation->vat,
                    'vat_option' => $quotation->vat_option,
                    'meta' => $quotation->meta,
                    'zone_id' => (function () use ($quotation) {
                        $meta = json_decode($quotation->meta, true) ?? [];
                        return $meta['zone_id'] ?? ($meta['location']['id'] ?? null);
                    })(),
                    'confirmed' => true,
                    'client_generated' => (bool) $quotation->client_generated,

                    //Generated by
                    'user_id' => $user->id,

                    'branch_id' => $quotation->branch_id ?? config('branch.default_id'),
                ]);
    ...
    {
        $last = Sale::withTrashed()->orderBy("code_alt", "desc")->first();
        if (is_object($last)) {
