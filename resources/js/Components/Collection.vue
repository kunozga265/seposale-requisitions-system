<template>
    <div @click="openDialog" class="flex justify-start items-center" :class="getStatusClass()">
        ...
        <div class="ml-3 letter-spacing-normal" style="letter-spacing: normal"
            :class="{ 'text-xl': !isSolo, 'text-xs': isSolo }">
    ...
    methods: {
        openDialog() {
            if (this.disabled != true) {
                //it was determined to be self collection
                if (this.product.delivery == null) {
                    this.showDialog = true
                }
                //if it does not exist
                else if (this.product.delivery.status == 3) {
                    this.showDialog = true

                } else {
                    this.$inertia.get(this.route('deliveries.show', { 'id': this.product.delivery.id }))
                }
            }
        },
        submit() {
