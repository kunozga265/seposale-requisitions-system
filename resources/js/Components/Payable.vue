            ...
            <table class="table-fixed w-full text-gray-500 dark:text-gray-400">
                <tr>
                    <th class="w-10 p-2 text-center">
                        <input type="checkbox" @change="togglePayables" v-model="checkPayables"
                ...
                <tr class="border-b" v-for="(item, index) in payable" :key="index">
                    <td class="w-10 text-center">
                        <input type="checkbox" @change="checkSelectAll" v-model="item.checked"
                            :disabled="item.requestForm != null" v-if="item.requestForm == null"
                            class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600" />
                        <span v-else>
                    ...
                    <td v-if="item.requestForm != null" class="p-2 text-left">
                        <inertia-link :href="route('request-forms.show', { id: item.requestForm?.data.id })" class=" ">{{
                            item.requestForm.data.code }}</inertia-link>
                    </td>
                    <td v-else class="p-2 text-left">
                        <span>-</span>
                    </td>
                    <td class="p-2 text-left">
                        <inertia-link v-if="item.delivery != null"
                            :href="route('clients.show', { id: item.delivery?.data.client.id })" class=" ">
                    ...
                    <td class="p-2 text-left">
                        <inertia-link v-if="item.delivery != null"
                            :href="route('deliveries.show', { id: item.delivery?.data.id })" class=" ">
    ...
    computed: {
        getTotal() {
            ...
            for (let x in this.payable) {
                sum += parseFloat(this.payable[x].total)
            }
            return sum
        },
        getItemsTotal() {
            ...
            for (let x in this.form.payables) {
                sum += parseFloat(this.form.payables[x].total)
            }
            return sum
        }
