    ...
    <app-layout>
            ...
            <template #content>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-2 mb-4">
                    ...
                    <div class="mb-2">
                        ...
                        <jet-input id="person" type="text" class="block w-full" v-model="form.fromTo"
                            autocomplete="person" />
                    </div>
                    <div class="mb-2">
        ...
        <dialog-modal :show="transferDialog" @close="transferDialog = false">
            ...
            <template #content>
                ...
                <div class="mb-4">
                    ...
                    <div v-if="!transferValidation" class="flex items-center w-full text-red">
                        <div class="text-sm text-red"><i class="mdi mdi-alert-circle text-red"></i> {{ transferError }}
                        </div>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-2">
                    ...
                    <div class="mb-4">
                        <jet-label for="reference" value="Reference" />
                        <jet-input id="reference" type="text" class="block w-full" v-model="form.reference_from"
                            autocomplete="bank-reference" />
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-2">
                    ...
                    <div class="mb-4">
                        <jet-label for="reference" value="Reference" />
                        <jet-input id="reference" type="text" class="block w-full" v-model="form.reference_to"
                            autocomplete="bank-reference" />
                    </div>
                </div>
                <div class="grid grid-cols-1 md:grid-cols-2 gap-2">
            ...
            <template #footer>
                ...
                <primary-button v-show="transferValidation" @click.native="transferMoney">
                    <svg v-show="form.processing" role="status" class="inline w-4 h-4 mr-3 text-white animate-spin"
                        viewBox="0 0 100 101" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <path
                            d="M100 50.5908C100 78.2051 77.6142 100.591 50 100.591C22.3858 100.591 0 78.2051 0 50.5908C0 22.9766 22.3858 0.59082 50 0.59082C77.6142 0.59082 100 22.9766 100 50.5908ZM9.08144 50.5908C9.08144 73.1895 27.4013 91.5094 50 91.5094C72.5987 91.5094 90.9186 73.1895 90.9186 50.5908C90.9186 27.9921 72.5987 9.67226 50 9.67226C27.4013 9.67226 9.08144 27.9921 9.08144 50.5908Z"
                            ...
                            <div class="card">
                                <div class="p-2 mb-2 relative ">
                                    ...
                                    <table class="w-full  text-left text-gray-500 dark:text-gray-400">
                                        ...
                                        <tbody class="pt-8">
                                            <tr v-for="(record, index) in filteredRecords.data" :key="index"
                                                class="border-b cursor-pointer hover:bg-gray-100 transition ease-in-out duration-200">
                                                <td class="p-2 text-left ">{{ getDate(record.date * 1000) }}</td>
                                                ...
                                                <td class="p-2 text-left " :class="{'text-green-500 font-bold': account.data.type == 'DEBIT', 'text-red-500 font-bold': account.data.type != 'DEBIT'}">{{
                                                    record.type === "DEBIT" ? numberWithCommas(record.amount.toFixed(2))
                                                    : ''
                                                    }}
                                                </td>
                                                <td class="p-2 text-left " :class="{'text-green-500 font-bold': account.data.type == 'CREDIT', 'text-red-500 font-bold': account.data.type != 'CREDIT'}">{{
...
export default {
    ...
    computed: {
        ...
        filteredRecords() {
            let filtered = this.account.data.records
