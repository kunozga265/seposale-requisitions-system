  ...
  <app-layout>
    ...
    <template #breadcrumbs>
      <li aria-current="page">
        <div class="flex items-center">
          ...
          <span class="heading-font uppercase text-sm font-medium text-gray-500 dark:text-gray-400">Deliveries</span>
        </div>
      </li>
    </template>
    ...
    <dialog-modal :show="filterDialog" @close="filterDialog = false">
      ...
      <template #footer>
        ...
        <primary-button class="ml-2" @click.native="filterDeliveries" :disabled="form.processing">
    ...
    <div class="mx-9 flex">
      ...
      <a :href="route('deliveries.index', { filter: 'processing' })">
        <div class="ml-1 flex items-center rounded-full py-2 px-3 bg-gray-200 text-gray-600 text-xs font-bold"
          :class="{ 'warning': headline === 'processing' }">
          <div>Processing</div>
          <i v-show="headline === 'processing'" class="ml-2 mdi mdi-check-circle text-gray-600  cursor"></i>
        </div>
      </a>
      <a :href="route('deliveries.index', { filter: 'delivered' })">
        <div class="ml-1 flex items-center rounded-full py-2 px-3 bg-gray-200 text-gray-600 text-xs font-bold "
          :class="{ 'success': headline === 'delivered' }">
          <div>Delivered</div>
          <i v-show="headline === 'delivered'" class="ml-2 mdi mdi-check-circle text-gray-600  cursor"></i>
        </div>
      </a>
      <a :href="route('deliveries.index', { filter: 'completed' })">
        <div class="ml-1 flex items-center rounded-full py-2 px-3 bg-gray-200 text-gray-600 text-xs font-bold "
          :class="{ 'success': headline === 'completed' }">
          <div>Completed</div>
          <i v-show="headline === 'completed'" class="ml-2 mdi mdi-check-circle text-gray-600  cursor"></i>
        </div>
      </a>
      <a :href="route('deliveries.index', { filter: 'cancelled' })">
        <div class="ml-1 flex items-center rounded-full py-2 px-3 bg-gray-200 text-gray-600 text-xs font-bold "
          :class="{ 'error': headline === 'cancelled' }">
          <div>Cancelled</div>
          <i v-show="headline === 'cancelled'" class="ml-2 mdi mdi-check-circle text-gray-600  cursor"></i>
        </div>
      </a>
    </div>
      ...
      <div class="max-w-7xl mx-auto px-2 sm:px-6 lg:px-8">
        ...
        <!-- <div class="page-section">
          ...
          <div class="page-section-content">
            ...
            <div v-if="deliveries.data.length === 0" class="text-center text-gray-400 md:col-span-2 text-sm">
              No Deliveries Found
            </div>
            <div v-else>
              <div class="grid grid-cols-1">
                <delivery v-for="(delivery, index) in deliveries.data" :key="index" :delivery="delivery" />
              </div>
            ...
            <div v-else>
              <div class="card">
                <div class="p-2 relative overflow-x-auto">
                  <table class="overflow-auto w-full default-table text-sm text-left text-gray-500 dark:text-gray-400">
                    <thead class=" text-gray-600 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                      <tr>
                        <th scope="col" class="p-2 pb-0 heading-font text-left">Due Date</th>
                        ...
                        <!-- <th scope="col" class="p-2 pb-0 heading-font text-left">Ordered Date</th> -->
                        <th scope="col" class="p-2 pb-0 heading-font text-right">Delivered</th>
                        <th scope="col" class="p-2 pb-0 heading-font text-right">Remaining</th>
                        <th scope="col" class="p-2 pb-0 heading-font text-right">Total</th>
                      </tr>
                    </thead>
                    <tbody>
                      <tr
                        class="border-b dark:bg-gray-800 dark:border-gray-700 odd:bg-white even:bg-gray-50 odd:dark:bg-gray-800 even:dark:bg-gray-700"
                        v-for="(delivery, index) in deliveries.data" :key="index">
                        <td class="py-2 pr-1 text-left">
                          {{ getDate(delivery.date * 1000) }}
                        </td>
                        <td @click="navigateToDelivery(delivery.id)" class=" cursor-pointer hover:bg-gray-50 ">
                          <delivery-status :product-compound="delivery.summary" :due="delivery.due" :is-solo="true"
                        ...
                        <td @click="navigateToClient(delivery.client.id)"
                        ...
                        <td class="py-2 pr-1 text-right">
                          {{ numberWithCommas(delivery.summary.quantity) }}
                        </td>
                      </tr>
                    </tbody>
                  </table>
