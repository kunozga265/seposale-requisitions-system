    ...
    <app-layout>
        ...
        <dialog-modal :show="showDialog" @close="showDialog = false">
            ...
            <template #content>
                <div class="delivery-profile grid grid-cols-1 md:grid-cols-2 gap-2">
                    ...
                    <div v-show="!noteValidation" class="flex items-center w-full text-red md:col-span-2">
                        <div class="text-sm text-red"><i class="mdi mdi-alert-circle text-red"></i> {{
                            addNoteErrorMessage }}</div>
                    </div>
            ...
            <template #footer>
                ...
                <primary-button v-show="delivery.data.status === 1 && noteValidation" @click.native="submit">
        ...
        <dialog-modal :show="requisitionDialog" @close="requisitionDialog = false">
            ...
            <template #content>
                ...
                <div>
                    <div class="flex justify-start items-center">
                        ...
                        <label @click="form.expenses.transportation.check = !form.expenses.transportation.check"
                            for="amount" class="ml-2 block mb-2 text-sm font-medium text-gray-900 dark:text-gray-300">
                ...
                <div>
                    <div class="flex justify-start items-center">
                        <input checked id="backdate" type="checkbox" value="" v-model="form.expenses.other.check"
                            class="w-4 h-4 mb-2 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600">
        ...
        <dialog-modal :show="completeDialog" @close="completeDialog = false">
            ...
            <template #content>
                <div class="mb-4">
                    ...
                    <div v-if="!payableValidation" class="flex items-center w-full text-red">
                        <div class="text-sm text-red"><i class="mdi mdi-alert-circle text-red"></i> {{ payableError }}
                        </div>
                    </div>
                    <div v-else-if="form.payables.length == 0" class="flex items-center w-full text-red">
                        <div class="text-sm text-red"><i class="mdi mdi-alert-circle text-red"></i> You have not added
                            any payables</div>
                    </div>
                </div>
                ...
                <div v-if="payableCheck === 'transporter'" class="grid grid-cols-2 gap-2 mb-4">
                    ...
                    <vue-date-time-picker color="#1a56db" v-model="transporter.date" :min-date="minDate" />
                ...
                <div v-else-if="payableCheck == 'supplier'" class="grid grid-cols-2 gap-2 mb-4">
                    ...
                    <vue-date-time-picker color="#1a56db" v-model="supplier.date" :min-date="minDate" />
            ...
            <template #footer>
                ...
                <primary-button v-show="delivery.data.status === 2 && payableValidation" @click.native="complete">
                    <svg v-show="form.processing" role="status" class="inline w-4 h-4 mr-3 text-white animate-spin"
                        viewBox="0 0 100 101" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <path
                            d="M100 50.5908C100 78.2051 77.6142 100.591 50 100.591C22.3858 100.591 0 78.2051 0 50.5908C0 22.9766 22.3858 0.59082 50 0.59082C77.6142 0.59082 100 22.9766 100 50.5908ZM9.08144 50.5908C9.08144 73.1895 27.4013 91.5094 50 91.5094C72.5987 91.5094 90.9186 73.1895 90.9186 50.5908C90.9186 27.9921 72.5987 9.67226 50 9.67226C27.4013 9.67226 9.08144 27.9921 9.08144 50.5908Z"
        ...
        <dialog-modal :show="cancelDialog" @close="cancelDialog = false">
            <template #title>
            ...
            <template #footer>
                <secondary-button @click.native="cancelDialog = false">
                    Cancel
                </secondary-button>
        ...
        <dialog-modal :show="deleteDialog" @close="deleteDialog = false">
            <template #title>
                Delete Delivery of {{ productName(delivery.data.summary) }}
            </template>

            <template #content>
                Are you sure you want to delete this delivery? All related accounting transactions will be reversed.
            </template>
            ...
            <template #footer>
                ...
                <danger-button class="ml-2" @click.native="deleteDelivery">
                    <svg v-show="form.processing" role="status" class="inline w-4 h-4 mr-3 text-white animate-spin"
                        viewBox="0 0 100 101" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <path
                            d="M100 50.5908C100 78.2051 77.6142 100.591 50 100.591C22.3858 100.591 0 78.2051 0 50.5908C0 22.9766 22.3858 0.59082 50 0.59082C77.6142 0.59082 100 22.9766 100 50.5908ZM9.08144 50.5908C9.08144 73.1895 27.4013 91.5094 50 91.5094C72.5987 91.5094 90.9186 73.1895 90.9186 50.5908C90.9186 27.9921 72.5987 9.67226 50 9.67226C27.4013 9.67226 9.08144 27.9921 9.08144 50.5908Z"
                ...
                <div>
                                    ...
                                    <inertia-link :href="route('sales.show', { id: delivery.data.summary.sale.id })">
                                        <div class="mb-4">
                                            ...
                                            <div class="text-gray-500 text-sm">
                                                {{ delivery.data.summary.sale.code }}
                                            </div>
                                        </div>
                                    </inertia-link>
                    ...
                    <div class="page-section" v-if="requestForms.data.length > 0">
                        ...
                        <div class="page-section-content">
                            <div class="grid grid-cols-1 md:grid-cols-2">
                                <request v-for="(request, index) in requestForms.data" :key="index"
                                    :request="request" />
                            </div>
                        </div>
                    </div>
                    ...
                    <div v-if="delivery.data.notes != null" class="page-section">
                        ...
                        <div v-if="delivery.data.notes.length > 0" class="page-section-content">
                            <div class="">
                                <div class="card p-0" v-for="(note, index) in delivery.data.notes" :key="index">
...
export default {
    ...
    computed: {
        ...
        noteValidation() {
            ...
            if (this.form.recipientPhoneNumber.length === 0 || this.form.recipientPhoneNumber == "") {
                this.addNoteErrorMessage = "Enter recipient phone number"
                return false
            }
            if (this.form.photo.length === 0 || this.form.photo == "") {
                this.addNoteErrorMessage = "Upload delivery note photo"
                return false
            }
            if (this.form.photo.length > 0 && !this.form.photo.match(/\.(jpg|jpeg|png|gif)$/)) {
                this.addNoteErrorMessage = "Invalid photo format"
                return false
            }
    ...
    methods: {
        ...
        deleteDelivery() {
            this.form
                ...
                .delete(this.route('deliveries.destroy', { 'id': this.delivery.data.id }), {
                    // preserveScroll: true,
                    onSuccess: () => {
