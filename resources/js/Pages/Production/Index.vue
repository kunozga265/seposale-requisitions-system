<template>
    <app-layout>
        ...
        <template #actions>
            <div class="md:flex grid grid-cols-2 md:grid-cols-5 gap-1">
                ...
                <jet-dropdown align="right" width="48">
                    ...
                    <template #content>
                           <jet-dropdown-link @click.native="addDamagesDialog = true" as="button" class="text-left">
                            Record Damages
                        </jet-dropdown-link>
        ...
        <dialog-modal :show="addDamagesDialog" @close="addDamagesDialog = false">
            <template #title>
                Record Damages
            </template>
            ...
            <template #content>
                <jet-validation-errors class="mb-4" />
                  ...
                  <div v-show="!damagesValidation" class="flex items-center w-full text-red md:col-span-2 mb-4">
                    <div class="text-sm text-red"><i class="mdi mdi-alert-circle text-red"></i> {{
                        damagesErrorMessage }}</div>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-2">
                    ...
                    <div class="mb-2 md:col-span-2">
                        ...
                        <select v-model="form.damagesInventoryId" id="type"
                            class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white"
                            required>
                            <option value="0">---</option>
                            <option v-for="(inventory, index) in inventories.data" :value="inventory.id" :key="index">
                                {{ inventory.name }}
                            </option>
                        </select>
                    </div>

                     <div class="mb-2" v-if="form.damagesInventoryId != 0">
                        <jet-label for="date" value="Date" />
                        <vue-date-time-picker color="#1a56db" v-model="form.damagesDate" :max-date="maxDate" />
                    </div>
                    <div class="mb-2" v-if="form.damagesInventoryId != 0">
                        <jet-label for="damagesQuantity" value="Quantity" />
                        <jet-input type="number" step="0.01" class="block w-full" v-model="form.damagesQuantity" required />
                    </div>
            ...
            <template #footer>
                <secondary-button @click.native="addDamagesDialog = false">
                    Cancel
                </secondary-button>

                <primary-button v-show="damagesValidation" class="ml-2" @click.native="storeDamages" :disabled="form.processing">
...
export default {
    ...
    computed: {
        ...
        damagesValidation() {
            if (this.form.damagesInventoryId == 0) {
                this.damagesErrorMessage = "Select product"
                return false
            }
            if (this.form.damagesDate == null) {
                this.damagesErrorMessage = "Select date"
                return false
            }
            if (this.form.damagesQuantity == 0 || this.form.damagesQuantity == null) {
                this.damagesErrorMessage = "Enter quantity of damages"
                return false
            }

            this.damagesErrorMessage = ""
    ...
    methods: {
        ...
        storeDamages() {
            this.form
                .transform(data => ({
                    ...data,
                    date: (new Date(this.form.damagesDate).getTime()) / 1000,
                    inventory_id: this.form.damagesInventoryId,
                    quantity: this.form.damagesQuantity,
                }))
                .post(this.route('damages.store'), {
                    preserveScroll: true,
                    onSuccess: () => {
                        this.addDamagesDialog = false
                        this.form.reset()
                    },
                })
