    ...
    <template #breadcrumbs>
      <li aria-current="page">
        <div class="flex items-center">
          ...
          <span
              class="heading-font uppercase text-sm font-medium text-gray-500 dark:text-gray-400">Suppliers</span>
        </div>
      </li>
    </template>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2">
          ...
          <div class="p-2 mb-2 md:col-span-2">
            <select v-model="form.id"
              ...
              <option :value="supplier.id" v-for="(supplier, index) in suppliers.data" :key="index">
                {{ supplier.name }}
              </option>
            </select>
          </div>
  ...
  computed: {
    ...
    priceValidation() {
      if (this.variantIndex < 0) {
        this.priceError = "Select supplier"
        return false
  ...
  watch: {
    variantIndex() {
      if (this.variantIndex < 0) {
        ...
        this.variantCost = this.filteredSuppliers[this.variantIndex].cost
      }
    }
  },
  methods: {
    ...
    editPrice() {
      this.form
          .transform(data => ({
            ...data,
            id: this.filteredSuppliers[this.variantIndex].variant_id,
            cost: this.variantCost,
          }))
          .post(this.route('suppliers.edit-price'), {
            onSuccess: () => {
