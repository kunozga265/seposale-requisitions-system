<template>
    <app-layout>
        ...
        <template #breadcrumbs>
            ...
            <li aria-current="page">
                <div class="flex items-center">
                    ...
                    <span
                        class="heading-font uppercase text-sm font-medium text-gray-500 dark:text-gray-400">Profile</span>
                </div>
            </li>
        </template>

        <template #actions>
            <primary-button v-if="checkRole(user.data, 'unverified') && checkRole($page.props.auth.data, 'management')"
                @click.native="verifyDialog = true">Verify</primary-button>
            <danger-button v-if="checkRole(user.data, 'unverified') && checkRole($page.props.auth.data, 'management')"
                @click.native="discardDialog = true">Discard</danger-button>
            <danger-button
                v-if="!checkRole(user.data, 'unverified') && !checkRole(user.data, 'disabled') && checkRole($page.props.auth.data, 'management')"
                @click.native="disableDialog = true">Disable</danger-button>
        </template>
        ...
        <dialog-modal :show="discardDialog" @close="discardDialog = false">
            ...
            <template #content>
                Are you sure you want to discard {{ user.data.firstName }} {{ user.data.middleName }}
                {{ user.data.lastName }}?
        ...
        <dialog-modal :show="disableDialog" @close="disableDialog = false">
            ...
            <template #content>
                Are you sure you want to disable {{ user.data.firstName }} {{ user.data.middleName }}
                {{ user.data.lastName }}?
        ...
        <dialog-modal :show="giveRoleDialog" @close="giveRoleDialog = false">
            ...
            <template #content>
                Are you sure you want to make this user {{ user.data.firstName }} {{ user.data.middleName }}
                {{ user.data.lastName }} act as
                {{ role }}?
            </template>
        ...
        <dialog-modal :show="revokeRoleDialog" @close="revokeRoleDialog = false">
            ...
            <template #content>
                Are you sure you want to revoke {{ user.data.firstName }} {{ user.data.middleName }} {{ user.data.lastName }}
                of the
                {{ role }} role?
        ...
        <div class="py-6">
            <div class="max-w-7xl mx-auto px-2 sm:px-6 lg:px-8">
                <div v-if="!checkRole(user.data, 'unverified') && checkRole($page.props.auth.data, 'management')"
                    class="page-section">
                        ...
                        <div class="card profile">
                            <div>
                                ...
                                <primary-button class="m-2"
                                    v-if="!checkRole(user.data, 'management') && checkRole($page.props.auth.data, 'management') && !checkRole(user.data, 'unverified') && !checkRole(user.data, 'disabled')"
                                    @click.native="giveRolePrompt('management')">Give Manager Role</primary-button>
                                <danger-button class="m-2"
                                    v-if="checkRole(user.data, 'management') && checkRole($page.props.auth.data, 'management') && !checkRole(user.data, 'unverified') && !checkRole(user.data, 'disabled')"
                                    @click.native="revokeRolePrompt('management')">Revoke Manager Role</danger-button>

                                <!--Accountant-->
                                <primary-button class="m-2"
                                    v-if="!checkRole(user.data, 'accountant') && checkRole($page.props.auth.data, 'management') && !checkRole(user.data, 'unverified') && !checkRole(user.data, 'disabled')"
                                    @click.native="giveRolePrompt('accountant')">Give Accountant Role</primary-button>
                                <danger-button class="m-2"
                                    v-if="checkRole(user.data, 'accountant') && checkRole($page.props.auth.data, 'management') && !checkRole(user.data, 'unverified') && !checkRole(user.data, 'disabled')"
                                    @click.native="revokeRolePrompt('accountant')">Revoke Accountant
                                    Role</danger-button>

                                <!--Administration-->
                                <primary-button class="m-2"
                                    v-if="!checkRole(user.data, 'administrator') && checkRole($page.props.auth.data, 'management') && !checkRole(user.data, 'unverified') && !checkRole(user.data, 'disabled')"
                                    @click.native="giveRolePrompt('administrator')">Give Administrator
                                    Role</primary-button>
                                <danger-button class="m-2"
                                    v-if="checkRole(user.data, 'administrator') && checkRole($page.props.auth.data, 'management') && !checkRole(user.data, 'unverified') && !checkRole(user.data, 'disabled')"
                        ...
                        <div class="card profile">
                            <div class="p-8 md:p-10 grid grid-cols-1 sm:grid-cols-2 gap-2">
                                ...
                                <div class="mb-4">
                                    ...
                                    <div>
                                        <span v-for="(role, index) in user.data.roles" :key="index"
                                            class="mr-2 role rounded py-1 px-2 bg-gray-200 text-gray-600 text-sm font-bold uppercase">
                                            {{ role.name }}
                                        </span>
                                    </div>
                            ...
                            <div class="h-64 w-64 sm:h-80 sm:w-80 mx-auto relative">
                                <div class="pie-chart absolute font-bold flex justify-center items-center">
                                    <div class="heading">
                                        ...
                                        <div class="xl font-bold heading-font text-center">{{
                                            totalRequests==1?'Request':'Requests' }}</div>
                                    </div>
                                </div>
                                <PieChart :chart-options="typesChartOptions" :chart-data="typesData" chart-id="types"
                            ...
                            <div class="card">
                                <div class="flex justify-start items-center">
                                    <div v-if="approvedRequestsCount > 0" class="overview-chart relative">
                                        ...
                                        <DoughnutChart :chart-options="chartOptions" :chart-data="approvedData"
                                            chart-id="approved" dataset-id-key="approved" />
                                    </div>
                                    <div class="ml-4">
                                        <div class="heading-font" style="font-weight: 600;">Approved</div>
                                        <div class="text-sm text-gray-400">{{ approvedRequestsCount }} {{
                                            approvedRequestsCount
                                            ==1?'Request':'Requests'}}</div>
                                    </div>
                                </div>
                            </div>
                            <div class="card">
                                <div class="flex justify-start items-center">
                                    <div v-if="pendingRequestsCount > 0" class="overview-chart relative">
                                        ...
                                        <DoughnutChart :chart-options="chartOptions" :chart-data="pendingData"
                                            chart-id="pending" dataset-id-key="pending" />
                                    </div>
                                    <div class="ml-4">
                                        <div class="heading-font" style="font-weight: 600;">Pending</div>
                                        <div class="text-sm text-gray-400">{{ pendingRequestsCount }} {{
                            ...
                            <div class="card">
                                <div class="flex justify-start items-center">
                                    <div v-if="deniedRequestsCount > 0" class="overview-chart relative">
                                        ...
                                        <DoughnutChart :chart-options="chartOptions" :chart-data="deniedData"
                                            chart-id="denied" dataset-id-key="denied" />
                                    </div>
                                    <div class="ml-4">
                                        <div class="heading-font" style="font-weight: 600;">Denied</div>
                                        <div class="text-sm text-gray-400">{{ deniedRequestsCount }} {{
                                            deniedRequestsCount
                                            ==1?'Request':'Requests'}}</div>
                                    </div>
                                </div>
                            ...
                            <div class="card">
                                <div class="flex justify-start items-center">
                                    <div v-if="closedRequestsCount > 0" class="overview-chart relative">
                                        ...
                                        <DoughnutChart :chart-options="chartOptions" :chart-data="completedData"
                                            chart-id="complete" dataset-id-key="complete" />
                                    </div>
                                    <div class="ml-4">
                                        <div class="heading-font" style="font-weight: 600;">Closed</div>
                                        <div class="text-sm text-gray-400">{{ closedRequestsCount }} {{
                                            closedRequestsCount
                                            ==1?'Request':'Requests'}}</div>
                                    </div>
                                </div>
                ...
                <div v-if="!checkRole(user.data, 'unverified')" class="page-section">
                    ...
                    <div class="page-section-content grid grid-cols-1 md:grid-cols-2">
                        <request v-for="(request, index) in activeRequests.data" :key="index" :request="request" />
                        <div v-if="activeRequests.data.length === 0"
                ...
                <div v-if="!checkRole(user.data, 'unverified')" class="page-section">
                    ...
                    <div class="page-section-content grid grid-cols-1 md:grid-cols-2">
                        <request v-for="(request, index) in closedRequests.data" :key="index" :request="request" />
                    </div>
                    <div v-if="closedRequests.data.length === 0"
                        class="text-center text-gray-400 md:col-span-2 text-sm">
                        No Requests Closed
                    </div>
                    <div v-else>
                        <div class="flex flex-col items-center">
                            ...
                            <span class="text-sm text-gray-700 dark:text-gray-400">
                                Showing <span class="font-semibold text-gray-900 dark:text-white">{{
                                    ...
                                    closedRequests.meta.to }}</span> of <span
                                    class="font-semibold text-gray-900 dark:text-white">{{ closedRequests.meta.total
                                    }}</span>
                                Requests
                            </span>
                            <div class="inline-flex mt-2 xs:mt-0">
                                ...
                                <a :href="closedRequests.links.next"
