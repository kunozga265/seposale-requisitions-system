    ...
    <template #breadcrumbs>
      <li aria-current="page">
        <div class="flex items-center">
          ...
          <span
              class="heading-font uppercase text-sm font-medium text-gray-500 dark:text-gray-400">Vacancys</span>
        </div>
      </li>
    </template>
      ...
      <div class="max-w-7xl mx-auto px-2 sm:px-6 lg:px-8">
        ...
        <div class="page-section">
          ...
          <div class="page-section-content">
            ...
            <div v-if="vacancies.data.length === 0"
                ...
                <div class="p-2 mb-2 relative ">
                  ...
                  <table class="w-full  text-left text-gray-500 dark:text-gray-400">
                    ...
                    <tr>
                      ...
                      <th scope="col" class="p-2 pb-0 heading-font text-center">Applications</th>
                    </tr>
                    ...
                    <tr
                        @click="navigateToVacancy(vacancy.id)"
                        class="border-b cursor-pointer hover:bg-gray-100 transition ease-in-out duration-200"
                        v-for="(vacancy,index) in filteredVacancies" :key="index">
...
<!--                      </td>-->
                      ...
                      <td class="p-2 text-left ">{{ getDate(vacancy.date*1000) }}</td>
                      
                      <td class="p-2 text-left ">{{ vacancy.title }}</td>
                      <td class="p-2 text-center ">{{ numberWithCommas(vacancy.applications) }}</td>
...
export default {
  ...
  computed:{
    filteredVacancies() {
      let filtered = this.vacancies.data

      /* Filter Sales By Vacancy*/
      if (this.form.name.length !== 0) {
        filtered = (filtered).filter((vacancy) => {
          if(vacancy.alias){
            return vacancy.name.toLowerCase().includes(this.form.name.toLowerCase()) || vacancy.alias.toLowerCase().includes(this.form.name.toLowerCase())
          }else {
            return vacancy.name.toLowerCase().includes(this.form.name.toLowerCase())
          }
        })
      }
    ...
    list(){
      return this.listOfVacancies
    }
  },
  methods: {
    navigateToVacancy(id) {
      this.$inertia.get(this.route('vacancies.show', {'id': id}))
    },
    selectVacancy(id){
      (this.listOfVacancies).push(id)
      console.log(id)
    }
  }
}
